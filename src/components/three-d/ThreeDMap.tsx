...
export default function ThreeDMap({ workspaceCode, token }: Props) {
  ...
  useEffect(() => {
    ...
    map.on("load", () => {
      ...
      setStatus("Relieve sin rótulos, radares y S-300 cargados");
    });
  ...
  useEffect(() => {
    ...
    for (const coverage of coverages) {
      for (const suffix of ["fill", "line"]) {
        ...
        if (map.getLayer(id)) {
          map.setLayoutProperty(id, "visibility", visible[coverage.id] ? "visible" : "none");
        }
      }
    }
  }, [visible]);
  ...
  const setGroup = (predicate: (coverage: Coverage) => boolean, value: boolean) => {
    setVisible((current) => ({
      ...current,
      ...Object.fromEntries(coverages.filter(predicate).map((coverage) => [coverage.id, value])),
    }));
  };
  ...
  return (
    <main className="min-h-screen bg-slate-950 text-white">
      <header className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-700 bg-slate-900 p-4">
        <div>
          <p className="text-xs font-bold uppercase tracking-widest text-amber-300">A3 · VISOR GEOESPACIAL</p>
          <h1 className="text-2xl font-bold">Relieve 3D, radares y S-300</h1>
          <p className="text-sm text-slate-300">Vista limpia, sin nombres de localidades ni provincias, con relieve vertical exagerado.</p>
        </div>
      ...
      <div className="grid min-h-[calc(100vh-100px)] lg:grid-cols-[330px_1fr]">
        <aside className="overflow-y-auto border-r border-slate-700 bg-slate-900 p-4 lg:max-h-[calc(100vh-100px)]">
          ...
          <div className="mb-4 grid grid-cols-2 gap-2 text-xs">
            <button onClick={() => setGroup((c) => c.side === "propio", true)} className="rounded bg-blue-800 p-2">Radares propios</button>
            <button onClick={() => setGroup((c) => c.side === "propio", false)} className="rounded bg-slate-700 p-2">Ocultar propios</button>
            <button onClick={() => setGroup((c) => c.side === "enemigo" && c.kind === "radar", true)} className="rounded bg-orange-800 p-2">Radares enemigos</button>
            <button onClick={() => setGroup((c) => c.side === "enemigo" && c.kind === "radar", false)} className="rounded bg-slate-700 p-2">Ocultar enemigos</button>
            <button onClick={() => setGroup((c) => c.kind === "s300", true)} className="rounded bg-red-800 p-2">Mostrar S-300</button>
            <button onClick={() => setGroup((c) => c.kind === "s300", false)} className="rounded bg-slate-700 p-2">Ocultar S-300</button>
          </div>

          <h2 className="mb-2 font-bold">Coberturas visibles</h2>
          {coverages.map((coverage) => (
            <label key={coverage.id} className="mb-2 flex items-center gap-2 rounded border border-slate-700 p-2 text-sm">
              <input
                type="checkbox"
                checked={visible[coverage.id]}
                onChange={(event) => setVisible((current) => ({ ...current, [coverage.id]: event.target.checked }))}
              />
          ...
          <label className="mt-5 block text-sm font-bold">
            ...
            <input type="range" min="1" max="8" step="0.1" value={exaggeration} onChange={(event) => setExaggeration(Number(event.target.value))} className="mt-2 w-full" />
          </label>
          <label className="mt-4 block text-sm font-bold">
            Opacidad de coberturas: {Math.round(opacity * 100)} %
            <input type="range" min="0.08" max="0.75" step="0.01" value={opacity} onChange={(event) => setOpacity(Number(event.target.value))} className="mt-2 w-full" />
          </label>

          <div className="mt-5 rounded border border-amber-800 bg-amber-950/30 p-3 text-xs text-amber-100">
            <strong>Nota:</strong> la base cartográfica fue reemplazada por sombreado del modelo de elevación. Por eso no aparecen nombres, rutas ni límites administrativos.
          </div>
        </aside>
