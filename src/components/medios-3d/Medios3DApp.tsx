...
function F16TechnicalPanel({ medio, onSelectVariant, onConfigure }: { medio: Medio3D; onSelectVariant:(id:string)=>void; onConfigure:()=>void }) {
  ...
  return <div className="flex h-full flex-col rounded-2xl border border-white/10 bg-slate-950/80 p-5 shadow-2xl">
    ...
    <div className="mt-4 grid grid-cols-2 gap-2 text-xs">
      {[['Longitud',f16General.longitud],['Envergadura',f16General.envergadura],['Altura',f16General.altura],['Peso s/ combustible',f16General.pesoSinCombustible],['MTOW',f16General.mtow],['Combustible interno',f16General.combustibleInterno],['Velocidad máx.',f16General.velocidad],['Alcance ferry',f16General.alcanceFerry],['Techo',f16General.techo],['Límite',f16General.limiteG]].map(([k,v])=><div key={k} className="rounded-lg border border-white/10 bg-white/[.03] p-3"><p className="uppercase tracking-wider text-slate-500">{k}</p><p className="mt-1 font-bold text-slate-100">{v}</p></div>)}
    </div>
  ...
  return <main className="min-h-screen bg-slate-950 text-white">
    ...
    <div className="mx-auto max-w-[1800px] px-5 py-6 lg:px-8">
      ...
      <section className={`mt-3 grid gap-5 ${fichaVisible?"xl:grid-cols-[minmax(0,8fr)_minmax(360px,3fr)]":"grid-cols-1"}`}>
        ...
        {fichaVisible&&<div>{isF16?(configureMode?<LoadoutPanel medio={chosen} selectedStation={selectedStation} setSelectedStation={setSelectedStation} loadout={loadout} setLoadout={setLoadout} onClose={()=>{setConfigureMode(false);setSelectedStation(null)}} onSelectVariant={selectVariant}/>:<F16TechnicalPanel medio={chosen} onSelectVariant={selectVariant} onConfigure={()=>setConfigureMode(true)}/>):<div className="h-full rounded-2xl border border-white/10 bg-slate-950/80 p-5"><p className="text-xs font-black uppercase tracking-[.28em] text-cyan-300">Ficha técnica</p><h2 className="mt-2 text-3xl font-black">{chosen.nombre}</h2><p className="mt-1 text-slate-400">{chosen.categoria}</p><div className="mt-5 space-y-3 text-sm"><div className="rounded-xl border border-white/10 p-4"><span className="text-slate-500">Función ZEUS</span><p className="mt-1 font-bold">{chosen.funcion}</p></div><div className="grid grid-cols-2 gap-2"><div className="rounded-xl border border-white/10 p-4"><span className="text-slate-500">Cantidad ZEUS</span><p className="mt-1 font-bold">{chosen.cantidad??'No detallada'}</p></div>{chosen.ubicacion&&<div className="rounded-xl border border-white/10 p-4"><span className="text-slate-500">Ubicación</span><p className="mt-1 font-bold">{chosen.ubicacion}</p></div>}</div>{chosen.especificaciones&&chosen.especificaciones.length>0&&<div><div className="mb-2 flex items-center justify-between"><p className="text-xs font-black uppercase tracking-[.22em] text-cyan-300">Especificaciones técnicas</p><span className="text-[10px] uppercase text-slate-500">Referencia de fabricante</span></div><div className="grid grid-cols-2 gap-2">{chosen.especificaciones.map(item=><div key={item.etiqueta} className="rounded-lg border border-white/10 bg-white/[.025] p-3"><p className="text-[10px] uppercase tracking-wider text-slate-500">{item.etiqueta}</p><p className="mt-1 text-xs font-bold text-slate-100">{item.valor}</p></div>)}</div></div>}{chosen.armamentoPlan&&chosen.armamentoPlan.length>0&&<div className="rounded-xl border border-amber-300/20 bg-amber-300/[.035] p-4"><p className="text-xs font-black uppercase tracking-[.2em] text-amber-300">Armamento en Plan ZEUS</p><div className="mt-2 flex flex-wrap gap-2">{chosen.armamentoPlan.map(a=><span key={a} className="rounded-full border border-amber-300/20 bg-slate-900 px-2 py-1 text-[10px] font-bold text-amber-100">{a}</span>)}</div></div>}<div className={`rounded-xl border p-4 ${generic3d?'border-emerald-300/20 bg-emerald-300/[.04]':'border-cyan-300/15 bg-cyan-300/[.03]'}`}><span className={generic3d?'text-emerald-300':'text-cyan-300'}>{nivelText[chosen.nivelModelo]}</span><p className="mt-1 text-xs leading-5 text-slate-300">{generic3d?'Vista tridimensional disponible.':chosen.nivelModelo==='3d-zeus'?'Modelo 3D ZEUS generado para análisis visual y reemplazable por un modelo exacto cuando esté disponible.':'Referencia visual disponible.'}</p>{chosen.id==="meko360"&&<p className="mt-2 text-xs leading-5 text-amber-200/75">Render neutro, sin banderas, matrículas ni identificación nacional.</p>}</div>{chosen.notaTecnica&&<div className="rounded-xl border border-cyan-300/15 bg-cyan-300/[.03] p-4 text-xs leading-5 text-slate-400">{chosen.notaTecnica}</div>}</div></div>}</div>}
      </section>
