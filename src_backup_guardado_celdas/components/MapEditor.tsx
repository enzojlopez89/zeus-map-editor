...
type EstadoInteligencia =
  "pendiente" | "estimado" | "probable" | "confirmado" | "descartado";
...
export type ElementoOperacional = {
  ...
  attackRouteId?: string;
  attackMedium?: string;
  attackPointOrder?: number;
  attackPointRole?: "origen" | "ruta" | "impacto";
};
...
const ESTABLECIMIENTOS_EJERCITO_ENEMIGO: BaseMilitar[] = [
  ...
  },
  {
    ...
    escalon: "Brigada",
    detalle:
      "Fuerza terrestre enemiga. 1.ª Brigada de Montaña ubicada en Jujuy.",
  },
  {
...
const DATOS_DISTANCIA_AEREA: Record<string, DatosDistanciaAerea> = {
  "F-16C Block 40": {
    ...
    fuente: "orden",
    referencia: "Orden: 740 MN (valor superior del intervalo 500–740 MN)",
    permiteReabastecimiento: true,
  },
  "F-16D Block 42": {
    km: 1370,
    termino: "Radio de acción",
  ...
  "Hermes 450": {
    ...
    termino: "Radio de acción",
    fuente: "orden",
    referencia: "Orden: 150 MN",
  },
  "IAI Harpy": {
    km: 500,
    termino: "Alcance operativo de empleo único",
    fuente: "orden",
    referencia: "Orden: 500 km / 270 MN",
  },

  "C-130J": {
    km: 2000,
    termino: "Radio de acción de referencia",
    fuente: "externa",
    referencia: "Estimado como 50% del alcance publicado con carga",
  },
  "KC-130J": {
    km: 1833,
    termino: "Radio de acción de referencia",
    fuente: "externa",
    referencia: "Estimado como 50% del alcance publicado",
  },
  "LJ-60": {
    km: 2220,
    termino: "Radio de acción de referencia",
    fuente: "externa",
    referencia: "Estimado como 50% del alcance máximo publicado",
  },
  "LJ-60 MEDEVAC": {
    km: 2220,
    termino: "Radio de acción de referencia",
  ...
  "UH-1Y": {
    km: 220,
    termino: "Radio de combate",
    fuente: "externa",
    referencia: "Fabricante: 119 MN de radio de combate",
  },
  "T-6 Texan II": {
    km: 830,
    termino: "Radio de acción de referencia",
  ...
  "E-99M Erieye": {
    ...
    fuente: "externa",
    referencia:
      "Estimado sobre plataforma ERJ-145; la orden solo informa autonomía",
  },
  "KC-135": {
    km: 2778,
    termino: "Radio de acción de referencia",
    fuente: "externa",
    referencia: "Valor público de referencia para misión de reabastecimiento",
  },
  "CH-47F": {
    km: 370,
    termino: "Radio de acción de referencia",
    fuente: "externa",
    referencia: "Valor público de referencia",
  },
  "EC-130H Compass Call": {
    km: 2130,
    termino: "Radio de acción de referencia",
    fuente: "externa",
    referencia: "Estimado como 50% del alcance publicado de la plataforma",
  },
  ...
  "C-98A": {
    ...
    termino: "Radio de acción de referencia",
    fuente: "externa",
    referencia: "Estimado a partir de aeronave utilitaria equivalente",
  },
  "UH-1N": {
    km: 230,
    termino: "Radio de acción de referencia",
    fuente: "externa",
    referencia: "Valor público aproximado",
  },
  "Mirage 2000-5 Mk2": {
    km: 740,
    termino: "Radio de combate de referencia",
    fuente: "externa",
    referencia: "Valor público aproximado",
    permiteReabastecimiento: true,
  },
  ...
  "E-2C AEW&C": {
    ...
    termino: "Radio de acción de referencia",
    fuente: "externa",
    referencia: "Estimado como 50% del alcance de traslado publicado",
  },
  "Falcon DA-20": {
    km: 1700,
    termino: "Radio de acción de referencia",
  ...
  "Geran-2": {
    ...
    termino: "Alcance operativo de empleo único",
    fuente: "externa",
    referencia: "Estimación externa; no figura en la orden",
  },
};

function datosDistanciaAerea(nombre: string): DatosDistanciaAerea {
  return (
    DATOS_DISTANCIA_AEREA[nombre] ?? {
      km: 0,
      termino: "Radio de acción",
      fuente: "manual",
...
function inferirTipoPersonalizado(
  nombre: string,
  iconoTipo: string,
): TipoElemento {
  ...
  if (
    texto.includes("antiaerea") ||
    texto.includes("misil") ||
    texto.includes("defensa")
  )
    return "defensa";
  if (
    texto.includes("aeronave") ||
    texto.includes("ala_fija") ||
    texto.includes("ala_rotativa") ||
    texto.includes("helicoptero") ||
    texto.includes("uav")
  )
    return "aeronave";
  if (
    texto.includes("naval") ||
    texto.includes("buque") ||
    texto.includes("fragata") ||
    texto.includes("corbeta") ||
    texto.includes("submarino") ||
    texto.includes("barco") ||
    texto.includes("superficie")
  )
    return "naval";
  return "terrestre";
}
...
const CATALOGO_AERONAVES_PROPIAS: AeronaveCatalogo[] = Object.entries(
  ...
  aeronaves.map((nombre) => ({
    ...
    permiteReabastecimiento:
      datosDistanciaAerea(nombre).permiteReabastecimiento,
  })),
);
...
const CATALOGO_AERONAVES_ENEMIGAS: AeronaveCatalogo[] = Object.entries(
  ...
  aeronaves.map((medio) => ({
    ...
    permiteReabastecimiento: datosDistanciaAerea(medio.nombre)
      .permiteReabastecimiento,
  })),
);
...
const CATALOGO_DEFENSA_ENEMIGA: MedioCatalogo[] = [
  ...[
    [
      ...
      6.5,
      undefined,
      "Sistema SA-29; cantidad no indicada en la orden",
    ],
    [
      "Ala Aérea n.º 2 / Sáenz Peña",
      "ZSU-23-2",
      2.5,
      undefined,
      "Sistema ZSU-23-2; no se aplica el alcance del ZSU-23-4 Shilka por tratarse de otro modelo",
    ],
    [
      "Ala Aérea n.º 2 / Sáenz Peña",
      "SA-8 Osa-AK",
    ...
    [
      ...
      70,
      2,
      "Radares asociados: AN/MPQ-46 / AN/MPQ-61",
    ],
    ...
    [
      ...
      2.5,
      undefined,
      "Modelo indicado por la orden; distinto del ZSU-23-4 Shilka",
    ],
    [
      "Ala Aérea n.º 3 / Salta",
      "SA-29",
      6.5,
      undefined,
      "Cantidad no indicada en la orden",
    ],
    ...
    [
      ...
      2.5,
      undefined,
      "La orden no confirma que sea ZSU-23-4 Shilka",
    ],
    [
      "Ala Aérea n.º 4 / Catamarca",
      "SA-29",
    ...
    const referencias: Record<string, string> = {
      "S-300 PMU-1":
        'Dato aportado para el ejercicio: 30N6E "Flap Lid", 150 km / 81 MN',
      "Improved HAWK":
        "Dato aportado para el ejercicio: AN/MPQ-46 / AN/MPQ-61, 70 km / 38 MN",
      "Pantsir-S1": "Dato aportado para el ejercicio: 1RS2-1E, 28 km / 15 MN",
      "SA-8 Osa-AK":
        "Dato aportado para el ejercicio: Land Roll, 30 km / 16 MN",
      "ZSU-23-4 Shilka":
        'Dato aportado para el ejercicio: RPK-2 "Gun Dish", 20 km / 11 MN',
    };
...
export default function MapEditor({
  ...
  const modoAtaqueRef = useRef(false);

  const [mapReady, setMapReady] = useState(false);
  const [elementos, setElementos] =
    useState<ElementoOperacional[]>(initialElements);
  ...
  const [vistaFuerzas, setVistaFuerzas] = useState<VistaFuerzas>(
    (ajustesIniciales.vistaFuerzas as VistaFuerzas | undefined) ?? "ambas",
  );

  const [mostrarRepublicas, setMostrarRepublicas] = useState(
    capasIniciales.republicas ?? true,
  );
  const [mostrarEntornoGeografico, setMostrarEntornoGeografico] = useState(
    capasIniciales.entornoGeografico ?? true,
  );
  const [mostrarTon, setMostrarTon] = useState(capasIniciales.ton ?? true);
  const [mostrarBases, setMostrarBases] = useState(
    capasIniciales.bases ?? true,
  );
  const [mostrarComunicaciones, setMostrarComunicaciones] = useState(
    capasIniciales.comunicaciones ?? false,
  );
  const [mostrarAeronaves, setMostrarAeronaves] = useState(
    capasIniciales.aeronaves ?? true,
  );
  const [mostrarRadares, setMostrarRadares] = useState(
    capasIniciales.radares ?? true,
  );
  const [mostrarDefensa, setMostrarDefensa] = useState(
    capasIniciales.defensa ?? true,
  );
  const [mostrarRelieve, setMostrarRelieve] = useState(
    capasIniciales.relieve ?? false,
  );
  const [mostrarRios, setMostrarRios] = useState(capasIniciales.rios ?? false);
  const [mostrarGrilla, setMostrarGrilla] = useState(
    capasIniciales.grilla ?? false,
  );
  const [intervaloGrilla, setIntervaloGrilla] = useState(
    Number(ajustesIniciales.intervaloGrilla ?? 2),
  );
  const [mostrarDimensionesTon, setMostrarDimensionesTon] = useState(
    capasIniciales.dimensionesTon ?? false,
  );
  const [modoMedicion, setModoMedicion] = useState(false);
  const [ordenPaneles, setOrdenPaneles] = useState<PanelId[]>(
    initialState?.panelOrder?.length
      ? initialState.panelOrder
      : ORDEN_PANELES_INICIAL,
  );
  ...
  const [modoAtaqueActivo, setModoAtaqueActivo] = useState(false);
  const [medioAtaque, setMedioAtaque] = useState(
    (ajustesIniciales.medioAtaque as string | undefined) ??
      "F-16CJ / AGM-88C HARM",
  );
  const [puntosAtaque, setPuntosAtaque] = useState<PuntoAtaque[]>(() =>
    ((ajustesIniciales.puntosAtaque as PuntoAtaque[] | undefined) ?? []).map(
      (punto, indice) => ({
        id: punto.id ?? crearId(),
        orden: punto.orden ?? indice + 1,
        coordenadas: punto.coordenadas,
      }),
    ),
  );
  const [latitudAtaque, setLatitudAtaque] = useState("");
  const [longitudAtaque, setLongitudAtaque] = useState("");
  ...
  >(
    () =>
      (ajustesIniciales.mascarasVisibles as
  ...
  >(
    () =>
      (ajustesIniciales.coloresMascaras as
  ...
  const [coloresRepublicas, setColoresRepublicas] = useState<
    Record<string, string>
  >(
    (ajustesIniciales.coloresRepublicas as
      Record<string, string> | undefined) ?? {
      ...COLORES_REPUBLICAS,
    },
  );
  ...
  const [basesVisibles, setBasesVisibles] = useState<Record<string, boolean>>(
    () =>
      ...
      Object.fromEntries(
        TODAS_LAS_BASES_Y_ESTABLECIMIENTOS.map((base) => [base.nombre, true]),
      ),
  );

  const [mostrarLaboratorioTritio, setMostrarLaboratorioTritio] =
    useState<boolean>(
      () =>
  ...
  const [mostrarCOAe, setMostrarCOAe] = useState<boolean>(
    () => (ajustesIniciales.mostrarCOAe as boolean | undefined) ?? true,
  );

  const [mostrarCOAeEnemigo, setMostrarCOAeEnemigo] = useState<boolean>(
    () => (ajustesIniciales.mostrarCOAeEnemigo as boolean | undefined) ?? true,
  );
  ...
  const iconosFiltrados = useMemo(() => {
    ...
    return catalogoIconos.filter((icono) => {
      const coincideBando =
  ...
  function crearAnillosElemento(elemento: ElementoOperacional) {
    ...
    if (radio > 0 && debeMostrarAnillo(elemento)) {
      anillos.push(
        circle([elemento.longitude, elemento.latitude], radio, {
          steps: 128,
          units: "kilometers",
          properties: {
            id: elemento.id,
  ...
  function agregarMedio(
    ...
    const nuevoElemento: ElementoOperacional = {
      ...
      cantidad: medio.cantidad,
      descripcion: medio.descripcion,
      terminoDistancia: medio.terminoDistancia ?? "Alcance máximo del sistema",
      fuenteDistancia: medio.fuenteDistancia,
      referenciaDistancia: medio.referenciaDistancia,
    };
  ...
  function agregarMedioPersonalizado() {
    ...
    const tipoInferido = inferirTipoPersonalizado(
      nombre,
  ...
  function actualizarFuentePlanAtaque(lista: PuntoAtaque[]) {
    const map = mapRef.current;
    if (!map) return;

    const source = map.getSource("plan-ataque") as
      maplibregl.GeoJSONSource | undefined;

    if (!source) return;

    const features: GeoJSON.Feature[] = lista.map((punto, indice) => ({
      type: "Feature",
      properties: {
        id: punto.id,
        orden: indice + 1,
        etiqueta: `${indice + 1}`,
        tipo:
          indice === lista.length - 1 && lista.length >= 2
            ? "impacto"
            : "punto",
        medio: medioAtaque,
      },
    ...
    if (lista.length >= 2) {
      features.unshift({
        type: "Feature",
        properties: { tipo: "ruta", medio: medioAtaque },
        geometry: {
          type: "LineString",
          coordinates: lista.map((punto) => punto.coordenadas),
        },
      });
    }
  ...
  function elementosPlanAtaqueActual(): ElementoOperacional[] {
    const routeId = `${workspaceCode ?? "mapa"}-plan-ataque`;

    return puntosAtaque.map((punto, indice) => {
      const rol =
        indice === 0
          ? "origen"
          : indice === puntosAtaque.length - 1 && puntosAtaque.length >= 2
            ? "impacto"
            : "ruta";
      ...
      return {
        ...
        bando: "propio",
        tipo: "aeronave",
        nombre: `${medioAtaque || "Medio de ataque"} / ${rol === "impacto" ? "Impacto" : `Punto ${indice + 1}`}`,
        baseOrigen: "Planificador de ataque aéreo",
        longitude: punto.coordenadas[0],
        latitude: punto.coordenadas[1],
        radioCombateKm: 0,
        alcanceKm: 0,
        mostrarAnillo: false,
        color: rol === "impacto" ? "#f97316" : "#ef4444",
        descripcion:
          rol === "impacto"
            ? "Punto de impacto / explosión del ataque planificado."
            : "Punto de ruta del ataque planificado.",
        operationTask: "Ataque aéreo",
        operationMission: "Plan de ataque trazado por la célula",
        operationNotes: `Ruta de ataque con ${puntosAtaque.length} punto(s). Medio: ${medioAtaque}.`,
        sharedWithCommander: true,
        sharedWithJem: true,
        sharedWithOtherCells: false,
        classification: "uso_interno",
        originWorkspaceCode: workspaceCode,
        attackRouteId: routeId,
        attackMedium: medioAtaque,
        attackPointOrder: indice + 1,
        attackPointRole: rol,
      };
    });
  }

  function actualizarFuentePlanesAtaqueCompartidos(listaElementos: ElementoOperacional[]) {
    const map = mapRef.current;
    if (!map) return;

    const source = map.getSource("planes-ataque-compartidos") as
      maplibregl.GeoJSONSource | undefined;

    if (!source) return;

    const puntos = listaElementos.filter(
      (elemento) =>
        elemento.sharedExternal &&
        elemento.attackRouteId &&
        typeof elemento.attackPointOrder === "number",
    );

    const porRuta = new Map<string, ElementoOperacional[]>();
    puntos.forEach((elemento) => {
      const id = elemento.attackRouteId ?? "ruta";
      porRuta.set(id, [...(porRuta.get(id) ?? []), elemento]);
    });

    const features: GeoJSON.Feature[] = [];

    porRuta.forEach((grupo, routeId) => {
      const ordenados = [...grupo].sort(
        (a, b) => (a.attackPointOrder ?? 0) - (b.attackPointOrder ?? 0),
      );
      ...
      if (ordenados.length >= 2) {
        features.push({
          ...
          properties: {
            tipo: "ruta",
            routeId,
            medio: ordenados[0]?.attackMedium ?? "Ataque aéreo",
            origen: ordenados[0]?.originWorkspaceCode ?? "CELDA",
          },
          geometry: {
            type: "LineString",
            coordinates: ordenados.map((elemento) => [
              elemento.longitude,
              elemento.latitude,
            ]),
          },
      ...
      ordenados.forEach((elemento) => {
        const rol = elemento.attackPointRole ?? "ruta";
        features.push({
          type: "Feature",
          properties: {
            tipo: rol === "impacto" ? "impacto" : "punto",
            routeId,
            orden: elemento.attackPointOrder ?? 0,
            etiqueta: `${elemento.originWorkspaceCode ?? "CELDA"}-${elemento.attackPointOrder ?? ""}`,
            medio: elemento.attackMedium ?? elemento.nombre,
          },
          geometry: {
            type: "Point",
            coordinates: [elemento.longitude, elemento.latitude],
          },
        });
      });
    });
  ...
  function iniciarPlanAtaque() {
    modoAtaqueRef.current = true;
    setModoAtaqueActivo(true);
    setModoMedicion(false);
    modoMedicionRef.current = false;
  }

  function detenerCargaPlanAtaque() {
    modoAtaqueRef.current = false;
    setModoAtaqueActivo(false);
  }

  function limpiarPlanAtaque() {
    setPuntosAtaque([]);
    actualizarFuentePlanAtaque([]);
    detenerCargaPlanAtaque();
  }

  function agregarPuntoAtaque(coordenadas: PuntoMedicion) {
    setPuntosAtaque((anteriores) => {
      const nuevos = [
        ...anteriores,
        {
          id: crearId(),
          orden: anteriores.length + 1,
          coordenadas,
        },
      ];
      actualizarFuentePlanAtaque(nuevos);
      return nuevos;
    });
    setCambiosPendientes(true);
  }

  function agregarPuntoAtaqueManual() {
    const lat = Number(latitudAtaque.replace(",", "."));
    const lng = Number(longitudAtaque.replace(",", "."));

    if (Number.isNaN(lat) || Number.isNaN(lng)) {
      setMensajeGuardado("Coordenadas de ataque inválidas");
      return;
    }

    agregarPuntoAtaque([lng, lat]);
    setLatitudAtaque("");
    setLongitudAtaque("");
  }

  function quitarUltimoPuntoAtaque() {
    setPuntosAtaque((anteriores) => {
      const nuevos = anteriores.slice(0, -1).map((punto, indice) => ({
        ...punto,
        orden: indice + 1,
      }));
      actualizarFuentePlanAtaque(nuevos);
  ...
  function colocarMedioAtaqueEnPunto() {
    const ultimoPunto = puntosAtaque[puntosAtaque.length - 1];
    const lat = Number(latitudAtaque.replace(",", "."));
    const lng = Number(longitudAtaque.replace(",", "."));
    const coordenadas =
      !Number.isNaN(lat) && !Number.isNaN(lng)
        ? ([lng, lat] as PuntoMedicion)
        : ultimoPunto?.coordenadas;

    if (!coordenadas) {
      setMensajeGuardado(
        "Marcá un punto o cargá coordenadas para colocar el medio.",
      );
      return;
    }
    ...
    const nuevo: ElementoOperacional = {
      ...
      bando: "propio",
      tipo: "aeronave",
      nombre: medioAtaque.trim() || "Medio de ataque",
      baseOrigen: "Punto específico de ataque",
      ...
      mostrarAnillo: false,
      color: "#ef4444",
      descripcion: "Medio colocado desde el planificador de ataque.",
      operationTask: "Ataque aéreo",
      operationMission:
        "Ataque planificado sobre punto de impacto seleccionado",
      operationNotes:
        puntosAtaque.length >= 2
          ? `Ruta con ${puntosAtaque.length} puntos. El último punto representa impacto/explosión.`
          : "Medio colocado en punto específico.",
    };
  ...
  useEffect(() => {
    ...
    map.on("load", async () => {
      ...
      const marcadorCOAeEnemigo = new maplibregl.Marker({
        element: crearMarcadorEspecial(
          COAE_ENEMIGO_INGENIERO_JUAREZ,
          "C2",
          "#fca5a5",
        ),
        anchor: "center",
      ...
      GRUPOS_COMUNICACIONES.forEach((grupo) => {
        const marcador = new maplibregl.Marker({
          ...
          .addTo(map);
        habilitarMedicionSobreMarcador(marcador, [
          grupo.longitude,
          grupo.latitude,
        ]);
      ...
      laboratorioTritioRef.current = new maplibregl.Marker({
        ...
        .setLngLat([LABORATORIO_TRITIO.longitude, LABORATORIO_TRITIO.latitude])
        .setPopup(
          new maplibregl.Popup({
            offset: 25,
            maxWidth: "420px",
      ...
      map.addLayer({
        id: "plan-ataque-puntos",
        type: "circle",
        source: "plan-ataque",
        filter: ["==", ["get", "tipo"], "punto"],
        paint: {
          "circle-radius": 7,
          "circle-color": "#fef2f2",
      ...
      map.addLayer({
        id: "plan-ataque-impacto",
        type: "circle",
        source: "plan-ataque",
        filter: ["==", ["get", "tipo"], "impacto"],
        paint: {
          "circle-radius": 15,
          "circle-color": "#f97316",
          "circle-opacity": 0.9,
      ...
      map.addSource("planes-ataque-compartidos", {
        type: "geojson",
        data: {
          type: "FeatureCollection",
          features: [],
      ...
      map.addLayer({
        id: "planes-ataque-compartidos-ruta",
        type: "line",
        source: "planes-ataque-compartidos",
        filter: ["==", ["get", "tipo"], "ruta"],
        paint: {
          "line-color": "#f97316",
          "line-width": 3.2,
      ...
      map.addLayer({
        id: "planes-ataque-compartidos-puntos",
        type: "circle",
        source: "planes-ataque-compartidos",
        filter: ["==", ["get", "tipo"], "punto"],
        paint: {
      ...
      map.addLayer({
        id: "planes-ataque-compartidos-impacto",
        type: "circle",
        source: "planes-ataque-compartidos",
        filter: ["==", ["get", "tipo"], "impacto"],
        paint: {
      ...
      map.addLayer({
        id: "planes-ataque-compartidos-etiquetas",
        type: "symbol",
        source: "planes-ataque-compartidos",
        filter: [
          "any",
      ...
      actualizarFuentePlanAtaque(puntosAtaque);
      actualizarFuentePlanesAtaqueCompartidos(elementos);
      ...
      map.on("click", (evento) => {
        if (modoAtaqueRef.current) {
          agregarPuntoAtaque([evento.lngLat.lng, evento.lngLat.lat]);
          return;
        }
    ...
    return () => {
      ...
      Object.values(comunicacionesRef.current).forEach((marker) =>
        marker.remove(),
      );
      ...
      const popupHtml = `
        <div style="font-family:Arial;color:#f8fafc;background:#0f172a;border-radius:8px;line-height:1.5;min-width:300px;overflow:hidden">
          ...
          <div style="margin-top:7px;padding-top:7px;border-top:1px solid #475569">
            <strong>${
              elemento.tipo === "aeronave"
                ? (elemento.terminoDistancia ?? "Radio de acción")
                : "Alcance"
            }:</strong>
  ...
  useEffect(() => {
    ...
    const visibleCOAe =
      mostrarBases && mostrarCOAe && bandoVisible("propio", vistaFuerzas);
  ...
  useEffect(() => {
    ...
    marcador.getElement().style.display = visibleLaboratorioTritio
      ? "flex"
      : "none";
  ...
  useEffect(() => {
    Object.values(comunicacionesRef.current).forEach((marcador) => {
      marcador.getElement().style.display = mostrarComunicaciones
        ? "flex"
        : "none";
    });
  ...
  useEffect(() => {
    if (!mapReady) return;
    actualizarFuentePlanAtaque(puntosAtaque);
  }, [mapReady, puntosAtaque, medioAtaque]);

  useEffect(() => {
    modoAtaqueRef.current = modoAtaqueActivo;
  }, [modoAtaqueActivo]);
  ...
  function crearSnapshot(): ZeusMapSnapshot {
    ...
    return {
      ...
      elements: [
        ...elementos.filter((elemento) => !elemento.attackRouteId),
        ...elementosPlanAtaqueActual(),
      ],
    };
  }
  ...
  const firmaElementosPropios = useMemo(
    () =>
      JSON.stringify(elementos.filter((elemento) => !elemento.sharedExternal)),
    [elementos],
  );
  ...
  return (
    <div className="flex h-screen w-screen">
      <aside
        className={`flex w-[420px] flex-col overflow-y-auto bg-slate-950 p-5 text-white ${readOnly ? "workspace-readonly" : ""}`}
      >
        ...
        {workspaceLabel && (
          <h2
            className="mb-1 text-sm font-semibold text-cyan-300"
            style={{ order: -2 }}
          >
            {workspaceLabel}
          </h2>
        ...
        <div
          className="sticky top-0 z-20 mb-4 rounded-lg border border-slate-700 bg-slate-950/95 p-3 shadow-lg"
          style={{ order: -1 }}
        >
          <div className="flex items-center justify-between gap-2">
            <span
              className={`text-xs font-semibold ${cambiosPendientes ? "text-amber-300" : "text-emerald-300"}`}
            >
              {mensajeGuardado}
            </span>
            {!readOnly && onSave && (
        ...
        <section
          {...propiedadesPanel("fuerzas")}
          className="mb-5 cursor-move rounded bg-slate-900 p-4"
        >
        ...
        <section
          {...propiedadesPanel("referencia")}
          className="mb-5 cursor-move rounded bg-slate-900 p-4 text-sm"
        >
          <h2 className="mb-2 font-semibold">↕ Referencia de radios</h2>
          <div className="mb-2 flex items-center gap-2">
            <span className="h-3 w-3 rounded-full bg-green-600" />
            Valor tomado de la orden de instrucción
          </div>
          <div className="flex items-center gap-2">
            <span className="h-3 w-3 rounded-full bg-red-600" />
            Referencia externa o estimación
          </div>
        </section>

        <section
          {...propiedadesPanel("capas")}
          className="mb-5 cursor-move rounded bg-slate-900 p-4"
        >
          ...
          <p className="mt-2 text-xs text-slate-400">
            El COAe de Río Cuarto puede mostrarse u ocultarse desde Bases y
            estaciones visibles → Propias.
          </p>
        </section>

        <section
          {...propiedadesPanel("bases")}
          className="mb-5 cursor-move rounded bg-slate-900 p-4"
        >
                  ...
                  <button
                    ...
                    onClick={() =>
                      setBasesVisibles((a) => ({
                        ...a,
                        ...Object.fromEntries(
                          BASES_PROPIAS.map((b) => [b.nombre, true]),
                        ),
                      }))
                    }
                  ...
                  <button
                    ...
                    onClick={() =>
                      setBasesVisibles((a) => ({
                        ...a,
                        ...Object.fromEntries(
                          BASES_PROPIAS.map((b) => [b.nombre, false]),
                        ),
                      }))
                    }
                  ...
                  <button
                    ...
                    onClick={() =>
                      setBasesVisibles((a) => ({
                        ...a,
                        ...Object.fromEntries(
                          BASES_Y_ESTABLECIMIENTOS_ENEMIGOS.map((b) => [
                            b.nombre,
                            true,
                          ]),
                        ),
        ...
        >
              ...
              <div className="mb-2 flex items-center justify-between gap-2">
                <div>
                  ...
                  <p className="mt-1 text-xs text-slate-400">
                    Coberturas reales importadas de los KMZ. Al activarlas se
                    oculta el círculo nominal del S-300 para mostrar claramente
                    la geometría irregular condicionada por el terreno.
                  </p>
                </div>
                <div className="flex gap-2 text-xs">
                  <button
                    ...
                    onClick={() =>
                      ...
                      ).forEach((mascara) => cambiarMascara(mascara.id, true))
                    }
                  ...
                  <button
                    ...
                    onClick={() =>
                      MASCARAS_RADAR.filter(
                        (mascara) => mascara.categoria === "defensa_antiaerea",
                      ).forEach((mascara) => cambiarMascara(mascara.id, false))
                    }
                    className="rounded bg-slate-700 px-2 py-1"
                  >
                    Ninguna
          ...
          <p className="mt-3 text-xs text-slate-400">
            Las máscaras radar conservan sus archivos originales. La cobertura
            ALFA del S-300 fue corregida y trasladada al emplazamiento de Belén.
          </p>
        </section>

        <section
          {...propiedadesPanel("colores")}
          className="mb-5 cursor-move rounded bg-slate-900 p-4"
        >
        ...
        <section
          {...propiedadesPanel("medicion")}
          className="mb-5 cursor-move rounded border border-yellow-600 bg-slate-900 p-4"
        >
        ...
        <section
          {...propiedadesPanel("personalizado")}
          className="mb-5 cursor-move rounded border border-emerald-700 bg-slate-900 p-4"
        >
          ...
          <select
            ...
            onChange={(event) =>
              setBandoPersonalizado(event.target.value as Bando)
            }
            className="mb-3 w-full rounded bg-slate-800 p-2"
          >
          ...
          <p className="mb-3 rounded bg-slate-800 p-2 text-xs text-slate-300">
            El medio se ubicará inicialmente en el océano próximo al teatro de
            operaciones. Luego podés arrastrarlo al punto exacto. El tipo se
            infiere por el nombre o por el icono elegido.
          </p>
        ...
        {workspaceCode === "a3" && (
          <section className="mb-5 rounded border border-red-900 bg-slate-900 p-4">
            <h2 className="mb-3 font-semibold text-red-300">
              Planificador de ataque aéreo
            </h2>
            <label className="mb-3 block text-sm">
              <span className="mb-1 block text-slate-300">
                Medio / paquete de ataque
              </span>
              <input
                value={medioAtaque}
                onChange={(event) => {
                  setMedioAtaque(event.target.value);
                  setCambiosPendientes(true);
                }}
                className="w-full rounded bg-slate-800 p-2"
                placeholder="Ej.: F-16CJ / AGM-88C HARM"
              />
            </label>
            ...
            <div className="mb-3 grid grid-cols-2 gap-2 text-sm">
              <button
                type="button"
                onClick={
                  modoAtaqueActivo ? detenerCargaPlanAtaque : iniciarPlanAtaque
                }
                className={`rounded px-3 py-2 font-semibold ${modoAtaqueActivo ? "bg-red-700" : "bg-emerald-700"}`}
              >
                {modoAtaqueActivo ? "Detener marcado" : "Marcar ruta en mapa"}
              </button>
              <button
                type="button"
                onClick={colocarMedioAtaqueEnPunto}
                className="rounded bg-cyan-700 px-3 py-2 font-semibold"
              >
                Colocar medio en punto
              </button>
            </div>
            ...
            <div className="mb-3 grid grid-cols-2 gap-2 text-sm">
              <input
                value={latitudAtaque}
                onChange={(event) => setLatitudAtaque(event.target.value)}
                className="rounded bg-slate-800 p-2"
                placeholder="Latitud"
              />
              <input
                value={longitudAtaque}
                onChange={(event) => setLongitudAtaque(event.target.value)}
                className="rounded bg-slate-800 p-2"
            ...
            <div className="mb-3 grid grid-cols-3 gap-2 text-xs">
              <button
                type="button"
                onClick={agregarPuntoAtaqueManual}
                className="rounded bg-slate-700 px-2 py-2"
              >
                Agregar coordenada
              </button>
              <button
                type="button"
                onClick={quitarUltimoPuntoAtaque}
                className="rounded bg-slate-700 px-2 py-2"
              >
                Quitar último
              </button>
            ...
            <div className="rounded bg-slate-950 p-3 text-xs text-slate-300">
              <p>
                Ruta actual: <strong>{puntosAtaque.length}</strong> punto(s). La
                línea se muestra punteada y el último punto se representa como
                impacto/explosión cuando hay dos o más puntos.
              </p>
              {puntosAtaque.length > 0 && (
                <ol className="mt-2 max-h-36 list-decimal overflow-auto pl-5">
                  {puntosAtaque.map((punto, indice) => (
                    <li key={punto.id}>
                      {indice === puntosAtaque.length - 1 &&
                      puntosAtaque.length >= 2
                        ? "💥 Impacto: "
                        : "Punto: "}
                      {punto.coordenadas[1].toFixed(5)},{" "}
                      {punto.coordenadas[0].toFixed(5)}
                    </li>
                  ))}
                </ol>
              )}
        ...
        {seleccionado?.sharedExternal && (
          <section className="mb-5 rounded border border-amber-700 bg-amber-950/40 p-4 text-sm text-amber-200">
            <p className="font-semibold">
              Elemento compartido por{" "}
              {seleccionado.originWorkspaceCode?.toUpperCase() ?? "otra célula"}
            </p>
            <p className="mt-1 text-xs">
              Se muestra en la vista consolidada. No se guarda ni modifica desde
              este espacio.
            </p>
          </section>
        )}
        ...
        {workspaceCode === "a3" &&
          ...
          !seleccionado.sharedExternal && (
            <section
              {...propiedadesPanel("operaciones")}
              className="mb-5 space-y-4 rounded border border-cyan-900 bg-slate-900 p-4"
            >
              ...
              <label className="block text-sm">
                ...
                <input
                  value={seleccionado.operationMission ?? ""}
                  onChange={(e) =>
                    actualizarElemento(seleccionado.id, {
                      operationMission: e.target.value,
                    })
                  }
                  className="w-full rounded bg-slate-800 p-2"
                />
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="block text-sm">
                  <span className="mb-1 block text-slate-300">Fase</span>
                  <input
                    value={seleccionado.operationPhase ?? ""}
                    onChange={(e) =>
                      actualizarElemento(seleccionado.id, {
                ...
                <label className="block text-sm">
                  ...
                  <select
                    value={seleccionado.operationPriority ?? "normal"}
                    onChange={(e) =>
                      actualizarElemento(seleccionado.id, {
                        operationPriority: e.target.value,
                      })
                    }
                    className="w-full rounded bg-slate-800 p-2"
                  >
              ...
              <label className="block text-sm">
                <span className="mb-1 block text-slate-300">Tarea</span>
                <input
                  value={seleccionado.operationTask ?? ""}
                  onChange={(e) =>
                    actualizarElemento(seleccionado.id, {
                      operationTask: e.target.value,
                    })
                  }
                  className="w-full rounded bg-slate-800 p-2"
                />
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="block text-sm">
                  <span className="mb-1 block text-slate-300">Inicio</span>
                  <input
                    type="datetime-local"
                    value={seleccionado.operationStart ?? ""}
                    onChange={(e) =>
                      actualizarElemento(seleccionado.id, {
                        operationStart: e.target.value,
                      })
                    }
                    className="w-full rounded bg-slate-800 p-2"
                  />
                ...
                <label className="block text-sm">
                  <span className="mb-1 block text-slate-300">Fin</span>
                  <input
                    type="datetime-local"
                    value={seleccionado.operationEnd ?? ""}
                    onChange={(e) =>
                      actualizarElemento(seleccionado.id, {
                        operationEnd: e.target.value,
                      })
                    }
                    className="w-full rounded bg-slate-800 p-2"
                  />
              ...
              <label className="block text-sm">
                <span className="mb-1 block text-slate-300">Indicativo</span>
                <input
                  value={seleccionado.callSign ?? ""}
                  onChange={(e) =>
                    actualizarElemento(seleccionado.id, {
              ...
              <label className="block text-sm">
                <span className="mb-1 block text-slate-300">Observaciones</span>
                <textarea
                  rows={3}
                  value={seleccionado.operationNotes ?? ""}
                  onChange={(e) =>
                    actualizarElemento(seleccionado.id, {
        ...
        {workspaceCode === "a1" &&
          ...
          !seleccionado.sharedExternal && (
            <section
              {...propiedadesPanel("personal")}
              className="mb-5 space-y-4 rounded border border-pink-900 bg-slate-900 p-4"
            >
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="block text-sm">
                  <span className="mb-1 block text-slate-300">Asignados</span>
                  <input
                    type="number"
                    min="0"
                    value={seleccionado.personnelAssigned ?? 0}
                    onChange={(e) =>
                      actualizarElemento(seleccionado.id, {
                        personnelAssigned: Number(e.target.value),
                      })
                    }
                    className="w-full rounded bg-slate-800 p-2"
                  />
                ...
                <label className="block text-sm">
                  <span className="mb-1 block text-slate-300">Disponibles</span>
                  <input
                    type="number"
                    min="0"
                    value={seleccionado.personnelAvailable ?? 0}
                    onChange={(e) =>
                      actualizarElemento(seleccionado.id, {
                        personnelAvailable: Number(e.target.value),
                      })
                    }
                    className="w-full rounded bg-slate-800 p-2"
                  />
                ...
                <label className="block text-sm">
                  <span className="mb-1 block text-slate-300">Bajas</span>
                  <input
                    type="number"
                    min="0"
                    value={seleccionado.personnelCasualties ?? 0}
                    onChange={(e) =>
                      actualizarElemento(seleccionado.id, {
                        personnelCasualties: Number(e.target.value),
                      })
                    }
                    className="w-full rounded bg-slate-800 p-2"
                  />
                ...
                <label className="block text-sm">
                  <span className="mb-1 block text-slate-300">Reemplazos</span>
                  <input
                    type="number"
                    min="0"
                    value={seleccionado.personnelReplacements ?? 0}
                    onChange={(e) =>
                      actualizarElemento(seleccionado.id, {
                        personnelReplacements: Number(e.target.value),
                      })
                    }
                    className="w-full rounded bg-slate-800 p-2"
                  />
              ...
              <label className="block text-sm">
                ...
                <select
                  value={seleccionado.medicalStatus ?? "normal"}
                  onChange={(e) =>
                    actualizarElemento(seleccionado.id, {
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  checked={seleccionado.evacuationRequired ?? false}
                  onChange={(e) =>
                    actualizarElemento(seleccionado.id, {
                      evacuationRequired: e.target.checked,
                    })
                  }
                />{" "}
                Requiere evacuación sanitaria
              </label>
              <label className="block text-sm">
                <span className="mb-1 block text-slate-300">Novedades</span>
                <textarea
                  rows={3}
                  value={seleccionado.personnelNotes ?? ""}
                  onChange={(e) =>
                    actualizarElemento(seleccionado.id, {
                      personnelNotes: e.target.value,
                    })
                  }
                  className="w-full rounded bg-slate-800 p-2"
                />
        ...
        {workspaceCode === "a4" &&
          ...
          !seleccionado.sharedExternal && (
            <section
              {...propiedadesPanel("logistica")}
              className="mb-5 space-y-4 rounded border border-emerald-900 bg-slate-900 p-4"
            >
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="block text-sm">
                  ...
                  <input
                    ...
                    min="0"
                    max="100"
                    value={seleccionado.fuelPercent ?? 0}
                    onChange={(e) =>
                      actualizarElemento(seleccionado.id, {
                ...
                <label className="block text-sm">
                  <span className="mb-1 block text-slate-300">Armamento %</span>
                  <input
                    type="number"
                    min="0"
                    max="100"
                    value={seleccionado.ammunitionPercent ?? 0}
                    onChange={(e) =>
                      actualizarElemento(seleccionado.id, {
                        ammunitionPercent: Number(e.target.value),
                      })
                    }
                    className="w-full rounded bg-slate-800 p-2"
                  />
              ...
              <label className="block text-sm">
                ...
                <select
                  value={seleccionado.materialStatus ?? "operativo"}
                  onChange={(e) =>
                    actualizarElemento(seleccionado.id, {
              ...
              <label className="block text-sm">
                ...
                <input
                  type="number"
                  min="0"
                  value={seleccionado.transportAvailable ?? 0}
                  onChange={(e) =>
                    actualizarElemento(seleccionado.id, {
                      transportAvailable: Number(e.target.value),
                    })
                  }
                  className="w-full rounded bg-slate-800 p-2"
                />
              ...
              <label className="block text-sm">
                ...
                <select
                  value={seleccionado.resupplyPriority ?? "normal"}
                  onChange={(e) =>
                    actualizarElemento(seleccionado.id, {
                      resupplyPriority: e.target.value,
                    })
                  }
                  className="w-full rounded bg-slate-800 p-2"
                >
              ...
              <label className="block text-sm">
                ...
                <textarea
                  rows={3}
                  value={seleccionado.logisticsNotes ?? ""}
                  onChange={(e) =>
                    actualizarElemento(seleccionado.id, {
        ...
        {workspaceCode === "a5" &&
          ...
          !seleccionado.sharedExternal && (
            <section
              {...propiedadesPanel("comunicaciones")}
              className="mb-5 space-y-4 rounded border border-sky-900 bg-slate-900 p-4"
            >
              <h2 className="font-semibold text-sky-300">
                Situación de comunicaciones
              </h2>
              <label className="block text-sm">
                <span className="mb-1 block text-slate-300">Tipo de nodo</span>
                <input
                  value={seleccionado.communicationNodeType ?? ""}
                  onChange={(e) =>
                    actualizarElemento(seleccionado.id, {
                      communicationNodeType: e.target.value,
                    })
                  }
                  className="w-full rounded bg-slate-800 p-2"
                />
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="block text-sm">
                  <span className="mb-1 block text-slate-300">Red</span>
                  <input
                    value={seleccionado.networkName ?? ""}
                    onChange={(e) =>
                      actualizarElemento(seleccionado.id, {
              ...
              <label className="block text-sm">
                ...
                <select
                  value={seleccionado.linkStatus ?? "operativo"}
                  onChange={(e) =>
                    actualizarElemento(seleccionado.id, {
              ...
              <label className="block text-sm">
                ...
                <input
                  type="number"
                  min="0"
                  value={seleccionado.coverageKm ?? 0}
                  onChange={(e) =>
                    actualizarElemento(seleccionado.id, {
                      coverageKm: Number(e.target.value),
                    })
                  }
                  className="w-full rounded bg-slate-800 p-2"
                />
              ...
              <div className="grid grid-cols-2 gap-3">
                <label className="block text-sm">
                  <span className="mb-1 block text-slate-300">Cifrado</span>
                  <input
                    value={seleccionado.encryptionStatus ?? ""}
                    onChange={(e) =>
                      actualizarElemento(seleccionado.id, {
                ...
                <label className="block text-sm">
                  <span className="mb-1 block text-slate-300">Redundancia</span>
                  <input
                    value={seleccionado.redundancy ?? ""}
                    onChange={(e) =>
                      actualizarElemento(seleccionado.id, {
              ...
              <label className="block text-sm">
                ...
                <textarea
                  rows={3}
                  value={seleccionado.communicationsNotes ?? ""}
                  onChange={(e) =>
                    actualizarElemento(seleccionado.id, {
                      communicationsNotes: e.target.value,
                    })
                  }
                  className="w-full rounded bg-slate-800 p-2"
                />
        ...
        {workspaceCode === "a2" &&
          seleccionado &&
          seleccionado.bando === "enemigo" && (
            <section
              {...propiedadesPanel("inteligencia")}
              className="mb-5 space-y-4 rounded border border-red-900 bg-slate-900 p-4"
            >
              ...
              <label className="block text-sm">
                ...
                <select
                  ...
                  onChange={(event) =>
                    actualizarElemento(seleccionado.id, {
                      intelligenceStatus: event.target
                        .value as EstadoInteligencia,
                    })
                  }
                  className="w-full rounded bg-slate-800 p-2"
                >
              ...
              <label className="block text-sm">
                ...
                <select
                  ...
                  onChange={(event) =>
                    actualizarElemento(seleccionado.id, {
                      confidenceLevel: event.target.value as NivelConfianza,
                    })
                  }
                  className="w-full rounded bg-slate-800 p-2"
                >
              ...
              <label className="block text-sm">
                ...
                <input
                  type="datetime-local"
                  value={
                    seleccionado.informationDate
                      ? seleccionado.informationDate.slice(0, 16)
                      : ""
                  }
                  onChange={(event) =>
                    actualizarElemento(seleccionado.id, {
                      informationDate: event.target.value
              ...
              <label className="block text-sm">
                <span className="mb-1 block text-slate-300">
                  Observaciones de inteligencia
                </span>
                <textarea
                  value={seleccionado.intelligenceNotes ?? ""}
                  onChange={(event) =>
                    actualizarElemento(seleccionado.id, {
                      intelligenceNotes: event.target.value,
                    })
                  }
                  rows={4}
                  className="w-full rounded bg-slate-800 p-2"
                  ...
                  onChange={(event) =>
                    actualizarElemento(seleccionado.id, {
                      classification: event.target
                        .value as ClasificacionInteligencia,
                    })
                  }
                  className="w-full rounded bg-slate-800 p-2"
                >
              ...
              <div className="space-y-2 rounded bg-slate-800 p-3 text-sm">
                ...
                <label className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    checked={seleccionado.sharedWithCommander ?? false}
                    onChange={(event) =>
                      actualizarElemento(seleccionado.id, {
                        sharedWithCommander: event.target.checked,
                      })
                    }
                  />{" "}
                  Comandante
                ...
                <label className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    checked={seleccionado.sharedWithJem ?? false}
                    onChange={(event) =>
                      actualizarElemento(seleccionado.id, {
                        sharedWithJem: event.target.checked,
                      })
                    }
                  />{" "}
                  JEM
                ...
                <label className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    checked={seleccionado.sharedWithOtherCells ?? false}
                    onChange={(event) =>
                      actualizarElemento(seleccionado.id, {
                        sharedWithOtherCells: event.target.checked,
                      })
                    }
                  />{" "}
                  Otras células
                ...
                {seleccionado.classification === "restringido" && (
                  <p className="text-xs text-amber-300">
                    Los elementos restringidos no serán compartidos aunque las
                    casillas estén marcadas.
                  </p>
                )}
              </div>
            </section>
        ...
        {seleccionado &&
          ["a1", "a2", "a3", "a4", "a5"].includes(workspaceCode ?? "") &&
          !seleccionado.sharedExternal && (
            <section
              {...propiedadesPanel("compartir")}
              className="mb-5 space-y-3 rounded border border-violet-900 bg-slate-900 p-4"
            >
              <h2 className="font-semibold text-violet-300">
                Compartición del elemento
              </h2>
              <label className="block text-sm">
                <span className="mb-1 block text-slate-300">Clasificación</span>
                <select
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  ...
                  onChange={(e) =>
                    actualizarElemento(seleccionado.id, {
                      sharedWithCommander: e.target.checked,
                    })
                  }
                />{" "}
                Compartir con Comandante
              </label>
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  ...
                  onChange={(e) =>
                    actualizarElemento(seleccionado.id, {
                      sharedWithJem: e.target.checked,
                    })
                  }
                />{" "}
                Compartir con JEM
              ...
              <label className="flex items-center gap-2 text-sm">
                <input
                  ...
                  onChange={(e) =>
                    actualizarElemento(seleccionado.id, {
                      sharedWithOtherCells: e.target.checked,
                    })
                  }
                />{" "}
                Compartir con otras células
              </label>
              {seleccionado.classification === "restringido" && (
                <p className="text-xs text-amber-300">
                  La clasificación restringida anula toda compartición al
                  guardar.
                </p>
              )}
            </section>
        ...
        {seleccionado && (
          <section className="space-y-4 rounded bg-slate-900 p-4">
            ...
            {seleccionado.tipo === "aeronave" && (
              <div
                className={`rounded border p-3 text-sm ${seleccionado.fuenteDistancia === "orden" ? "border-green-600 bg-green-950/40" : "border-red-600 bg-red-950/40"}`}
              >
                <p
                  className={
                    seleccionado.fuenteDistancia === "orden"
                      ? "text-green-300"
                      : "text-red-300"
                  }
                >
                  ...
                  {seleccionado.fuenteDistancia === "orden"
                    ...
                    : seleccionado.fuenteDistancia === "externa"
                      ? "Referencia externa / estimación"
                      : "Carga manual"}
                </p>
                <p className="mt-1 text-xs text-slate-300">
                  {seleccionado.referenciaDistancia}
                </p>
                {seleccionado.permiteReabastecimiento && (
                  <label className="mt-3 flex items-center gap-2 text-cyan-200">
                    <input
                      ...
                      onChange={(event) =>
                        actualizarElemento(seleccionado.id, {
                          conReabastecimiento: event.target.checked,
                        })
                      }
                    />
                ...
                {seleccionado.conReabastecimiento && (
                  <p className="mt-2 text-xs text-cyan-300">
                    Radio representado con REV:{" "}
                    {formatearDistancia(radioAereoRepresentado(seleccionado))}.
                    Es una hipótesis simplificada de planeamiento.
                  </p>
                )}
              </div>
            )}
