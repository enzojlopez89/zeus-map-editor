...
export default function MapEditor() {
  ...
  function mediosDeBase(base: BaseMilitar) {
    ...
    return `
      <div style="font-family:Arial;color:#111827;min-width:260px">
  ...
  useEffect(() => {
    ...
    elementos.forEach((elemento) => {
      ...
      const popupHtml = `
        ...
        Bando: ${elemento.bando === "propio" ? "PROPIO" : "ENEMIGO"}<br />
        Tipo: ${elemento.tipo}<br />
        Base de origen: ${elemento.baseOrigen}<br />
        ${
          elemento.cantidad
            ? `Cantidad informada: ${elemento.cantidad}<br />`
            : ""
        }
        ${
          elemento.descripcion
            ? `Capacidad: ${elemento.descripcion}<br />`
            : ""
        }
        ${
          elemento.tipo === "aeronave"
            ? `Radio de combate: ${
                elemento.radioCombateKm > 0
                  ? `${elemento.radioCombateKm} km`
                  : "Sin definir"
              }`
            : `Alcance: ${elemento.alcanceKm} km`
        }
      `;
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  <input
                    type="number"
                    min={0}
                    value={seleccionado.radioCombateKm}
                    onChange={(event) =>
                      actualizarElemento(seleccionado.id, {
                        radioCombateKm: Math.max(
                          0,
