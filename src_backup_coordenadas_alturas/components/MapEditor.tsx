...
export type ElementoOperacional = {
  ...
  isAttackPlan?: boolean;
  attackName?: string;
  attackMedium?: string;
  attackRoute?: [number, number][];
  attackImpact?: [number, number];
  attackMunition?: string;
};
...
function formatearNumeroCoordenada(valor: number) {
  return valor.toFixed(6);
}

function formatearDms(valor: number, tipo: "lat" | "lon") {
  const direccion = tipo === "lat" ? (valor >= 0 ? "N" : "S") : valor >= 0 ? "E" : "O";
...
function coordenadasDetalladasHtml(
  latitude: number,
  longitude: number,
  altitudeMeters?: number,
) {
...
export default function MapEditor({
  ...
  const [medioAtaque, setMedioAtaque] = useState("F-16CJ + AGM-88C HARM");
  const [nombreAtaque, setNombreAtaque] = useState("Ataque planificado");
  const [municionAtaque, setMunicionAtaque] = useState("GBU / HARM / MAR-1");
  const [puntosAtaque, setPuntosAtaque] = useState<[number, number][]>([]);
  const [latAtaqueManual, setLatAtaqueManual] = useState("");
  const [lngAtaqueManual, setLngAtaqueManual] = useState("");
  ...
  useEffect(() => {
    ...
    const map = mapRef.current;
    if (map) {
      map.getCanvas().style.cursor = modoMedicion || modoAtaque ? "crosshair" : "";
    }
  }, [modoMedicion, modoAtaque]);

  useEffect(() => {
    modoAtaqueRef.current = modoAtaque;
  }, [modoAtaque]);
  ...
  function actualizarFuenteAtaques(puntosTemporales: [number, number][] = puntosAtaque) {
    const map = mapRef.current;
    const source = map?.getSource("planes-ataque") as maplibregl.GeoJSONSource | undefined;
    if (!source) return;

    const features: GeoJSON.Feature[] = [];

    elementos
      .filter((elemento) => elemento.isAttackPlan && elemento.visible !== false && elemento.attackRoute && elemento.attackRoute.length >= 2)
      .forEach((elemento) => {
        const ruta = elemento.attackRoute ?? [];
        features.push({
          type: "Feature",
          properties: { id: elemento.id, nombre: elemento.attackName ?? elemento.nombre, tipo: "ruta" },
          geometry: { type: "LineString", coordinates: ruta },
        });
        ruta.forEach((coordenada, indice) => {
          features.push({
            type: "Feature",
            properties: { id: elemento.id, nombre: elemento.attackName ?? elemento.nombre, tipo: indice === ruta.length - 1 ? "impacto" : "punto", numero: indice + 1 },
    ...
    if (puntosTemporales.length > 0) {
      puntosTemporales.forEach((coordenada, indice) => {
        features.push({
          type: "Feature",
          properties: { id: "ataque-temporal", nombre: nombreAtaque, tipo: indice === puntosTemporales.length - 1 ? "impacto-temporal" : "punto-temporal", numero: indice + 1 },
          geometry: { type: "Point", coordinates: coordenada },
        });
      });
      if (puntosTemporales.length >= 2) {
        features.push({
          type: "Feature",
          properties: { id: "ataque-temporal", nombre: nombreAtaque, tipo: "ruta-temporal" },
          geometry: { type: "LineString", coordinates: puntosTemporales },
        });
      }
    }
  ...
  function agregarPuntoAtaque(coordenadas: [number, number]) {
    if (!modoAtaqueRef.current || readOnly) return;
    setPuntosAtaque((anteriores) => {
      const nuevos = [...anteriores, coordenadas];
      actualizarFuenteAtaques(nuevos);
      return nuevos;
    });
  }

  function agregarPuntoAtaqueManual() {
    const lat = Number(latAtaqueManual.replace(",", "."));
    const lng = Number(lngAtaqueManual.replace(",", "."));
  ...
  function borrarRutaAtaqueTemporal() {
    setPuntosAtaque([]);
    actualizarFuenteAtaques([]);
  }

  function guardarAtaquePlanificado() {
    if (readOnly || puntosAtaque.length < 2) return;
    const origen = puntosAtaque[0];
    const impacto = puntosAtaque[puntosAtaque.length - 1];
    const id = `ataque-${Date.now()}`;
    const nuevoAtaque: ElementoOperacional = {
      id,
      visible: true,
      bando: "propio",
      tipo: "aeronave",
      nombre: nombreAtaque.trim() || `Ataque ${medioAtaque}`,
      baseOrigen: "Planificador de ataque",
      longitude: origen[0],
      latitude: origen[1],
      radioCombateKm: 0,
      alcanceKm: 0,
      mostrarAnillo: false,
      color: "#ef4444",
      descripcion: `Medio: ${medioAtaque}. Munición/efecto: ${municionAtaque}. Último punto: impacto/explosión sobre el blanco.`,
      classification: "compartible",
      sharedWithCommander: true,
      sharedWithJem: true,
      sharedWithOtherCells: true,
      originWorkspaceCode: workspaceCode,
      isAttackPlan: true,
      attackName: nombreAtaque.trim() || `Ataque ${medioAtaque}`,
      attackMedium: medioAtaque,
      attackRoute: puntosAtaque,
      attackImpact: impacto,
      attackMunition: municionAtaque,
    };

    setElementos((anteriores) => [...anteriores, nuevoAtaque]);
  ...
  useEffect(() => {
    ...
    map.on("load", async () => {
        ...
        .setPopup(
          new maplibregl.Popup({ offset: 25, maxWidth: "380px", className: "zeus-popup" }).setHTML(`
            <div style="background:#0f172a;color:white;padding:14px">
              ...
              ${coordenadasDetalladasHtml(COAE_RIO_CUARTO.latitude, COAE_RIO_CUARTO.longitude, COAE_RIO_CUARTO.altitudeMeters)}
            </div>
          `),
        )
        .addTo(map);
        ...
        .setPopup(
          new maplibregl.Popup({ offset: 25, maxWidth: "380px", className: "zeus-popup" }).setHTML(`
            <div style="background:#0f172a;color:white;padding:14px">
              ...
              ${coordenadasDetalladasHtml(COAE_ENEMIGO_INGENIERO_JUAREZ.latitude, COAE_ENEMIGO_INGENIERO_JUAREZ.longitude, COAE_ENEMIGO_INGENIERO_JUAREZ.altitudeMeters)}
            </div>
          `),
        )
        .addTo(map);
          ...
          .setPopup(
            new maplibregl.Popup({ offset: 25, maxWidth: "380px", className: "zeus-popup" }).setHTML(`
              <div style="background:#0f172a;color:white;padding:14px">
                ...
                <em>Bando propio</em>
                ${coordenadasDetalladasHtml(grupo.latitude, grupo.longitude, grupo.altitudeMeters)}
              </div>
            `),
          )
          .addTo(map);
        ...
        .setPopup(
          new maplibregl.Popup({
            ...
            <div style="background:#0f172a;color:white;padding:14px">
              ...
              <strong>Coordenadas decimales:</strong> ${formatearNumeroCoordenada(LABORATORIO_TRITIO.latitude)}, ${formatearNumeroCoordenada(LABORATORIO_TRITIO.longitude)}<br />
              <strong>Altura:</strong> ${LABORATORIO_TRITIO.altitudeMeters.toLocaleString("es-AR")} m s. n. m.<br />
      ...
      map.addSource("planes-ataque", {
        type: "geojson",
        data: { type: "FeatureCollection", features: [] },
      });

      map.addLayer({
        id: "planes-ataque-lineas",
        type: "line",
        source: "planes-ataque",
        filter: ["==", ["geometry-type"], "LineString"],
        paint: {
          "line-color": "#dc2626",
          "line-width": 3,
      ...
      map.addLayer({
        id: "planes-ataque-puntos",
        type: "circle",
        source: "planes-ataque",
        filter: ["all", ["==", ["geometry-type"], "Point"], ["!", ["in", ["get", "tipo"], ["literal", ["impacto", "impacto-temporal"]]]]],
        paint: {
          "circle-radius": 6,
          "circle-color": "#ffffff",
      ...
      map.addLayer({
        id: "planes-ataque-impactos-centro",
        type: "circle",
        source: "planes-ataque",
        filter: ["all", ["==", ["geometry-type"], "Point"], ["in", ["get", "tipo"], ["literal", ["impacto", "impacto-temporal"]]]],
        paint: {
      ...
      map.on("click", "planes-ataque-lineas", (evento) => {
        const id = evento.features?.[0]?.properties?.id;
        if (typeof id === "string" && id !== "ataque-temporal") setSeleccionadoId(id);
      });

      map.on("click", "planes-ataque-impactos", (evento) => {
        const id = evento.features?.[0]?.properties?.id;
        if (typeof id === "string" && id !== "ataque-temporal") setSeleccionadoId(id);
      });
    ...
    elementos.forEach((elemento) => {
      ...
      const popupHtml = `
        <div style="font-family:Arial;color:#f8fafc;background:#0f172a;border-radius:8px;line-height:1.5;min-width:300px;overflow:hidden">
          ...
          ${elemento.isAttackPlan ? `
            <div style="margin-top:8px;padding:8px;border:1px solid #ef4444;border-radius:8px;background:rgba(127,29,29,0.35)">
              <div style="font-weight:800;color:#fecaca">PLAN DE ATAQUE</div>
              <div><strong>Medio:</strong> ${elemento.attackMedium ?? "Sin definir"}</div>
              <div><strong>Munición / efecto:</strong> ${elemento.attackMunition ?? "Sin definir"}</div>
              <div><strong>Puntos de ruta:</strong> ${elemento.attackRoute?.length ?? 0}</div>
              <div><strong>Último punto:</strong> impacto / explosión sobre el blanco</div>
            </div>
          ` : ""}
  ...
  useEffect(() => {
    actualizarFuenteAtaques(puntosAtaque);
  }, [elementos, puntosAtaque, mapReady]);
      ...
      <aside className={`flex w-[420px] flex-col overflow-y-auto bg-slate-950 p-5 text-white ${readOnly ? "workspace-readonly" : ""}`}>
        ...
        {!readOnly && (
          <section className="mb-5 rounded border border-red-700 bg-slate-900 p-4">
            <h2 className="mb-3 font-semibold text-red-300">Planificador de ataque aéreo</h2>
            <label className="mb-2 block text-xs text-slate-300">Nombre del ataque</label>
            <input value={nombreAtaque} onChange={(e) => setNombreAtaque(e.target.value)} className="mb-3 w-full rounded bg-slate-800 p-2 text-sm" />
            <label className="mb-2 block text-xs text-slate-300">Medio empleado</label>
            <select value={medioAtaque} onChange={(e) => setMedioAtaque(e.target.value)} className="mb-3 w-full rounded bg-slate-800 p-2 text-sm">
              <option>F-16CJ + AGM-88C HARM</option>
              <option>F-16C/D + GBU-10/12/38</option>
              <option>AMX A-1M + MAR-1 / GBU</option>
              <option>IAI HARPY SEAD</option>
              <option>EC-130H Compass Call</option>
              <option>COMAO A3</option>
            </select>
            <label className="mb-2 block text-xs text-slate-300">Munición / efecto</label>
            <input value={municionAtaque} onChange={(e) => setMunicionAtaque(e.target.value)} className="mb-3 w-full rounded bg-slate-800 p-2 text-sm" />
            <button type="button" onClick={() => { setModoAtaque((actual) => !actual); if (modoMedicion) setModoMedicion(false); }} className={`mb-2 w-full rounded px-3 py-2 font-semibold ${modoAtaque ? "bg-red-500 text-white" : "bg-slate-700 text-white"}`}>
              {modoAtaque ? "Finalizar carga de puntos" : "Seleccionar puntos en el mapa"}
            </button>
            <div className="mb-2 grid grid-cols-2 gap-2">
              <input placeholder="Latitud" value={latAtaqueManual} onChange={(e) => setLatAtaqueManual(e.target.value)} className="rounded bg-slate-800 p-2 text-xs" />
              <input placeholder="Longitud" value={lngAtaqueManual} onChange={(e) => setLngAtaqueManual(e.target.value)} className="rounded bg-slate-800 p-2 text-xs" />
            </div>
            <button type="button" onClick={agregarPuntoAtaqueManual} className="mb-2 w-full rounded bg-slate-700 px-3 py-2 text-sm font-semibold">Agregar punto por coordenadas</button>
            <button type="button" onClick={guardarAtaquePlanificado} disabled={puntosAtaque.length < 2} className="mb-2 w-full rounded bg-red-700 px-3 py-2 text-sm font-bold disabled:opacity-40">Guardar ataque y compartir con Comandante/JEM</button>
            <button type="button" onClick={borrarRutaAtaqueTemporal} className="w-full rounded bg-slate-700 px-3 py-2 text-sm font-semibold">Borrar ruta temporal</button>
            <div className="mt-3 rounded bg-slate-800 p-3 text-xs">
              <div>Puntos marcados: <strong>{puntosAtaque.length}</strong></div>
              <div>El último punto guardado se representa como explosión sobre el blanco.</div>
              <div>La ruta se dibuja con línea punteada.</div>
            </div>
          </section>
        )}
        ...
        {seleccionado?.sharedExternal && (
          <section className="mb-5 space-y-3 rounded border border-amber-700 bg-amber-950/40 p-4 text-sm text-amber-100">
            <div>
              <p className="font-semibold text-amber-200">
                Elemento compartido por {seleccionado.originWorkspaceCode?.toUpperCase() ?? "otra célula"}
              </p>
              <p className="mt-1 text-xs text-amber-200/80">
                Se muestra en la vista consolidada. No se modifica desde este espacio.
              </p>
            </div>

            {(seleccionado.originWorkspaceCode === "a3" || seleccionado.operationMission || seleccionado.operationTask || seleccionado.operationPhase || seleccionado.operationNotes) && (
              <div className="rounded border border-cyan-800 bg-slate-950/70 p-3 text-slate-100">
                <p className="mb-2 font-semibold text-cyan-300">A3 Operaciones: acción / efecto</p>
                <div className="space-y-1 text-xs">
                  {seleccionado.operationMission && <p><strong>Misión / efecto:</strong> {seleccionado.operationMission}</p>}
                  {seleccionado.operationTask && <p><strong>Tarea:</strong> {seleccionado.operationTask}</p>}
                  {seleccionado.operationPhase && <p><strong>Fase:</strong> {seleccionado.operationPhase}</p>}
                  {seleccionado.operationPriority && <p><strong>Prioridad:</strong> {seleccionado.operationPriority}</p>}
                  {seleccionado.operationStart && <p><strong>Inicio:</strong> {seleccionado.operationStart}</p>}
                  {seleccionado.operationEnd && <p><strong>Fin:</strong> {seleccionado.operationEnd}</p>}
                  {seleccionado.callSign && <p><strong>Indicativo:</strong> {seleccionado.callSign}</p>}
                  {seleccionado.operationNotes && <p><strong>Observaciones:</strong> {seleccionado.operationNotes}</p>}
                  {!seleccionado.operationMission && !seleccionado.operationTask && !seleccionado.operationPhase && !seleccionado.operationNotes && (
                    <p className="text-slate-400">La célula A3 compartió el elemento, pero todavía no cargó acción, tarea o efecto.</p>
                  )}
                </div>
              </div>
            )}

            {(seleccionado.originWorkspaceCode === "a1" || seleccionado.personnelAssigned || seleccionado.personnelAvailable || seleccionado.personnelNotes || seleccionado.evacuationRequired) && (
              <div className="rounded border border-pink-800 bg-slate-950/70 p-3 text-slate-100">
                <p className="mb-2 font-semibold text-pink-300">A1 Personal: situación de efectivos</p>
                <div className="space-y-1 text-xs">
                  {seleccionado.personnelAssigned !== undefined && <p><strong>Asignados:</strong> {seleccionado.personnelAssigned}</p>}
                  {seleccionado.personnelAvailable !== undefined && <p><strong>Disponibles:</strong> {seleccionado.personnelAvailable}</p>}
                  {seleccionado.personnelCasualties !== undefined && <p><strong>Bajas:</strong> {seleccionado.personnelCasualties}</p>}
                  {seleccionado.personnelReplacements !== undefined && <p><strong>Reemplazos:</strong> {seleccionado.personnelReplacements}</p>}
                  {seleccionado.medicalStatus && <p><strong>Estado sanitario:</strong> {seleccionado.medicalStatus}</p>}
                  {seleccionado.evacuationRequired && <p><strong>Evacuación sanitaria:</strong> requerida</p>}
                  {seleccionado.personnelNotes && <p><strong>Novedades:</strong> {seleccionado.personnelNotes}</p>}
                </div>
              </div>
            )}

            {(seleccionado.originWorkspaceCode === "a2" || seleccionado.intelligenceStatus || seleccionado.intelligenceNotes || seleccionado.sourceDescription) && (
              <div className="rounded border border-red-800 bg-slate-950/70 p-3 text-slate-100">
                <p className="mb-2 font-semibold text-red-300">A2 Inteligencia: estado del dato</p>
                <div className="space-y-1 text-xs">
                  {seleccionado.intelligenceStatus && <p><strong>Estado:</strong> {seleccionado.intelligenceStatus}</p>}
                  {seleccionado.confidenceLevel && <p><strong>Confianza:</strong> {seleccionado.confidenceLevel}</p>}
                  {seleccionado.informationDate && <p><strong>Fecha:</strong> {seleccionado.informationDate}</p>}
                  {seleccionado.sourceDescription && <p><strong>Fuente:</strong> {seleccionado.sourceDescription}</p>}
                  {seleccionado.intelligenceNotes && <p><strong>Observaciones:</strong> {seleccionado.intelligenceNotes}</p>}
                </div>
              </div>
            )}

            {(seleccionado.originWorkspaceCode === "a4" || seleccionado.fuelPercent !== undefined || seleccionado.ammunitionPercent !== undefined || seleccionado.logisticsNotes) && (
              <div className="rounded border border-emerald-800 bg-slate-950/70 p-3 text-slate-100">
                <p className="mb-2 font-semibold text-emerald-300">A4 Logística: sostenimiento</p>
                <div className="space-y-1 text-xs">
                  {seleccionado.fuelPercent !== undefined && <p><strong>Combustible:</strong> {seleccionado.fuelPercent}%</p>}
                  {seleccionado.ammunitionPercent !== undefined && <p><strong>Armamento:</strong> {seleccionado.ammunitionPercent}%</p>}
                  {seleccionado.materialStatus && <p><strong>Estado del material:</strong> {seleccionado.materialStatus}</p>}
                  {seleccionado.transportAvailable !== undefined && <p><strong>Transportes disponibles:</strong> {seleccionado.transportAvailable}</p>}
                  {seleccionado.resupplyPriority && <p><strong>Prioridad de reabastecimiento:</strong> {seleccionado.resupplyPriority}</p>}
                  {seleccionado.logisticsNotes && <p><strong>Observaciones:</strong> {seleccionado.logisticsNotes}</p>}
                </div>
              </div>
            )}

            {(seleccionado.originWorkspaceCode === "a5" || seleccionado.communicationNodeType || seleccionado.networkName || seleccionado.communicationsNotes) && (
              <div className="rounded border border-sky-800 bg-slate-950/70 p-3 text-slate-100">
                <p className="mb-2 font-semibold text-sky-300">A5 Comunicaciones: enlace / red</p>
                <div className="space-y-1 text-xs">
                  {seleccionado.communicationNodeType && <p><strong>Tipo de nodo:</strong> {seleccionado.communicationNodeType}</p>}
                  {seleccionado.networkName && <p><strong>Red:</strong> {seleccionado.networkName}</p>}
                  {seleccionado.frequency && <p><strong>Frecuencia / canal:</strong> {seleccionado.frequency}</p>}
                  {seleccionado.linkStatus && <p><strong>Estado del enlace:</strong> {seleccionado.linkStatus}</p>}
                  {seleccionado.coverageKm !== undefined && <p><strong>Cobertura:</strong> {seleccionado.coverageKm} km</p>}
                  {seleccionado.encryptionStatus && <p><strong>Cifrado:</strong> {seleccionado.encryptionStatus}</p>}
                  {seleccionado.redundancy && <p><strong>Redundancia:</strong> {seleccionado.redundancy}</p>}
                  {seleccionado.communicationsNotes && <p><strong>Observaciones:</strong> {seleccionado.communicationsNotes}</p>}
                </div>
              </div>
            )}
          </section>
        ...
        {seleccionado && (
          <section className="space-y-4 rounded bg-slate-900 p-4">
            ...
            {seleccionado.isAttackPlan && (
              <div className="rounded border border-red-700 bg-red-950/30 p-3 text-sm">
                <h3 className="mb-2 font-semibold text-red-300">Ataque planificado</h3>
                <p><strong>Medio:</strong> {seleccionado.attackMedium}</p>
                <p><strong>Munición / efecto:</strong> {seleccionado.attackMunition}</p>
                <p><strong>Puntos de ruta:</strong> {seleccionado.attackRoute?.length ?? 0}</p>
                <p><strong>Impacto:</strong> {seleccionado.attackImpact ? `${seleccionado.attackImpact[1].toFixed(5)}, ${seleccionado.attackImpact[0].toFixed(5)}` : "Sin definir"}</p>
                <p className="mt-2 text-xs text-slate-300">El último punto de la ruta se visualiza como explosión sobre el blanco. Si está compartido, Comandante y JEM lo ven al guardar.</p>
              </div>
            )}
